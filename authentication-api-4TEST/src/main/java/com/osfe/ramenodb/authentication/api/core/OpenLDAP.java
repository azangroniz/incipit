...
public class OpenLDAP {
	...
	private static final String LDAP_SERVER = "ldap://101.101.53.150:389/dc=osfe,dc=org,dc=ar";

	public static Optional<User> validateOnlyUser() {
		Optional<User> foundUser;
		foundUser = returnDefaultUser();
		return foundUser;
	}

	public static Optional<User> returnDefaultUser() {
		//
		//// String searchFilter = "(&(objectClass=group)(uid=" + accountName +
		// "))";
		// String searchFilter = "(uid=usuario1)";
		...
		// }
		return Optional.of(new User("TEST", "TEST"));
	}
	...
	public static Optional<User> findUserByUniqueName(LdapContext ctx, String accountName) {
		...
		try {
			...
			// String grupos = getPrimaryGroupSID(searchResult);
			return createUser(searchResult);
		} catch (NamingException e) {
			return Optional.absent();
		}
	...
	// }
	public static List findGroup(String accountName, String group) {

		// String searchFilter = "(&(objectClass=group)(uid=" + accountName +
		// "))";
		accountName = "member=uid=" + accountName + ",ou=people,dc=osfe,dc=org,dc=ar";
		group = "CN=" + group + "";
		String filter = "(&(" + group + ")(" + accountName + "))";
		String ldapSearchBase = "";
		String[] returningAttrs = { "member" };

		// String[] returningAttrs = {
		// "givenname", "member", "samaccountname",
		// "userPrincipalName", "objectGUID"
		// };
		SearchControls searchControls = new SearchControls();
		searchControls.setSearchScope(SearchControls.SUBTREE_SCOPE);
		...
		try {
			...
			NamingEnumeration n = searchResult.getAttributes().get("member").getAll();
			for (int i = 0; i < searchResult.getAttributes().get("member").size(); i++) {
				l.add(n.next().toString());

			}
			// return
			// searchResult.getAttributes().get("member").getAll().nextElement();
			return l;
		} catch (Exception e) {
			e.printStackTrace();
			return null;
