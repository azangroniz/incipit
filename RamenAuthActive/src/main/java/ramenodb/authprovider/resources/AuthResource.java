...
import ramenodb.authprovider.auth.AuthUtils;
import ramenodb.authprovider.auth.PasswordService;
import ramenodb.authprovider.core.Token;
...
import ramenodb.authprovider.resources.AuthResource.Payload;
import ramenodb.authprovider.resources.AuthResource.UnlinkRequest;
...
public class AuthResource {
    ...
    public Response login(@Valid final User user, @Context final HttpServletRequest request)
        ...
        final Token token = AuthUtils.createToken(request.getRemoteHost(), foundUser.get().getUniqueName());
    ...
    public Response signup(@Valid final User user, @Context final HttpServletRequest request)
        ...
        final Token token = AuthUtils.createToken(request.getRemoteHost(), savedUser.getUniqueName());
    ...
    private Response processUser(final HttpServletRequest request, final Provider provider,
        ...
        if (StringUtils.isNotBlank(request.getHeader(AuthUtils.AUTH_HEADER_KEY))) {
            ...
            ldapUser = ActiveDirectory.findUserByName(subject);
...
//                userToSave = dao.merge(userToSave);
        ...
        final Token token = AuthUtils.createToken(request.getRemoteHost(), userToSave.getUniqueName());
